...
const APIkey = 'AIzaSyB1ENAhUg71WTj7Lb6XBciRImqHcD8A9Dc';
                    ...
                    $('.nextPage').click(function() {
                        $.ajax({
                            ...
                            success: function(data) {
                                ...
                                smJson = JSON.parse(data);
                                console.log("이건 더보기 눌렀을때 가져오는 data를 json 파싱한거");
                                console.log(smJson);
                                setTimeout(function() {
                                    $('.summary').each(function(index) { 
                                        summaryText = JSON.parse(smJson["summary"][index]);
                                        summaryText = summaryText[0]["result_sum_line3"];
...
function setSummary() {
    ...
    // else {
        setTimeout(function() {
            smJson = JSON.parse({{smJson|tojson|safe}});
            console.log(smJson);
            $('.summary').each(function(index) { 
                summaryText = JSON.parse(smJson["summary"][index]);
                summaryText = summaryText[0]["result_sum_line3"];
